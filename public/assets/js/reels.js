...
function renderReels(reels) {
  ...
  reels.forEach((reel, index) => {
    ...
    if (reel.is_ad) {
      ...
      reelEl.dataset.contextReelId = reel.context_reel_id || "";
    }
                ...
                .then(() => {
                  ...
                  if (!viewedReels.has(reelId)) {
                    video._viewTimer = setTimeout(() => {
                      ...
                      const contextReelId = entry.target.dataset.contextReelId;
                      trackReelView(
                        reelId,
                        hash,
                        isAd,
  ...
  if (isAd) {
    ...
    fetch("/api/ads/track", {
      ...
      body: JSON.stringify({
        ...
        reel_id: contextReelId || lastViewedReelId, // Context for revenue share
      }),
    }).catch(console.error);
    return;
  }
...
function initReels() {
  // Reset state for SPA navigation if on the main reels feed page
  if (document.getElementById("reels-feed")) {
    reelsPage = 1;
    reelsData = [];
    reelsHasMore = true;
    reelsLoading = false;
