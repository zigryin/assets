...
document.addEventListener("DOMContentLoaded",()=>{let lastVisibleUrl=window.location.href;let alertOpen=!1;let queue=[];const zigry={hooks:{beforeMount:[],afterMount:[],beforeNavigate:[],afterNavigate:[],onFormSubmit:[],onAssetsLoaded:[],},escapeHtml(str){if(!str)return"";const map={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;",};return str.replace(/[&<>"']/g,(m)=>map[m])},use(name,callback){if(!zigry.hooks[name])zigry.hooks[name]=[];zigry.hooks[name].push(callback)},runHooks(name,payload){(zigry.hooks[name]||[]).forEach((fn)=>fn(payload))},mount(html,props={}){const app=document.getElementById("zigry-app");let root;if(html){root=document.createElement("div");root.innerHTML=html}else{root=app||document}
...
const topLevel=comments.filter((c)=>!c.parent_id);const replies=comments.filter((c)=>c.parent_id);const replyMap={};replies.forEach((r)=>{if(!replyMap[r.parent_id])replyMap[r.parent_id]=[];replyMap[r.parent_id].push(r)});const renderComment=(c,isReply=!1)=>{const fallbackAvatar=`/assets/images/default/${
          ...
          }.png`;const avatarSrc=c.avatar||fallbackAvatar;const likedClass=c.is_liked?"text-danger":"text-muted";const likeIcon=c.is_liked?"<i class='zigry z-like zigry-md  fill-red-700'></i>":"<i class='zigry z-like zigry-md  no-fill stroke-red-700'></i>";let html=`
...
if(mediaData.videos){mediaData.videos.forEach((vid)=>{const posterAttr=vid.thumb?`poster="${vid.thumb}"`:"";const streamUrl=`/api/video/stream?path=${encodeURIComponent(
            vid.url,
          )}`;mediaHtml+=`<div class="zigry-video-wrapper border border-solid" style="position:relative;background-color: rgb(0, 0, 0);"><video class="w-100 zigry-video" playsinline ${posterAttr} src="${streamUrl}" preload="metadata">Your browser does not support video.</video></div>`})}}catch(e){mediaHtml=post.media||""}
div.innerHTML=`
          ...
          <div class="count d-flex justify-content-between mb-1">
            ...
            <div class="buttons d-flex btn-group-justified border-0 align-items-center">
              <div class="btn w-100 text-start align-items-center" data-action="like"><span class="fs-5 rounded like-icon text-danger">${
                post.likes ? "<i class='zigry z-like zigry-md fill-red-700'></i>" : "<i class='zigry zigry-md z-like no-fill stroke-red-700'></i>"
