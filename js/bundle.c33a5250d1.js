...
for(let[k,v]of Object.entries(props)){root.querySelectorAll(`[data-prop="${k}"]`).forEach((el)=>{if(typeof v==="object"&&v!==null){if(v.label)el.innerHTML=v.label;if(v.action)el.setAttribute("data-action",v.action);if(v.class)el.className=v.class}else{el.textContent=v}})}
...
async function commentPost(postId,card,button){let commentsSection=card.querySelector(".comments-section");if(!commentsSection){commentsSection=document.createElement("div");commentsSection.className="comments-section px-3 py-2 border-top";commentsSection.innerHTML=`
        ...
        <div class="comment-input-group d-flex gap-2 align-items-end">
          ...
          <button class="btn btn-sm btn-zigry comment-submit-btn" data-post-id="${postId}">Post</button>
        </div>
                    ...
                    <div class="d-flex gap-2">
                      ...
                      <div class="flex-grow-1">
                        ...
                        <div class="d-flex gap-3 mt-1 small text-muted">
                          ...
                          ${c.is_owner || c.user_id == window.currentUserId ? `<span class="comment-delete-btn text-danger" role="button" data-post-id="${postId}" data-comment-id="${c.id}" style="cursor:pointer;">Delete</span>` : ""}
                        </div>
                      </div>
                    </div>
                  </div>
...
const newEl=container.querySelector(`[data-comment-id="${c.id}"]`,);if(newEl){bindCommentActions(newEl)}
...
zigry.sharePost=sharePost;async function sharePost(postId,card,button,type="post"){const contentUrl=type==="reel"?`${window.location.origin}/reels/${postId}`:`${window.location.origin}/post/${postId}`;const modal=document.createElement("div");modal.className="modal fade custom-share-modal";modal.style.zIndex="20005";modal.innerHTML=`
      <style>
      ...
      <div class="modal-dialog modal-dialog-centered" style="z-index: 20006;">
        <div class="modal-content" style="background: #1a1a1a; color: #fff; border-radius: 16px;">
          ...
          <div class="modal-body px-3 pb-2">
            ...
            <div class="mb-3">
              <div class="input-group">
                <span class="input-group-text bg-dark border-0">
                  <i class="zigry z-search"></i>
                </span>
                <input type="text" class="form-control bg-dark border-0 text-white friends-search-input" 
            ...
            <div class="friends-grid-container" style="max-height: 350px; overflow-y: auto; margin: 0 -8px; scrollbar-width: none; -ms-overflow-style: none;">
              <style>
                .friends-grid-container::-webkit-scrollbar {
                  display: none;
                }
              </style>
              <div class="friends-loading text-center py-4">
                <div class="spinner-border spinner-border-sm text-light" role="status">
                  <span class="visually-hidden">Loading...</span>
                </div>
                <div class="small text-muted mt-2">Loading friends...</div>
              </div>
            </div>
          </div>

          <!-- Message Input -->
          <div class="px-3 pb-3" style="background: #1a1a1a;">
            <textarea 
              class="form-control bg-dark border-0 text-white share-message-input" 
              placeholder="Write a message..." 
              rows="2"
              autofocus
              style="resize: none; box-shadow: none; border-radius: 12px;">
            </textarea>
          </div>

          <!-- Bottom Action Buttons -->
          <div class="modal-footer border-0 justify-content-around p-3" style="background: #0a0a0a; border-radius: 0 0 16px 16px;">
            <button class="btn btn-link text-white text-decoration-none d-flex flex-column align-items-center share-to-story-btn" style="font-size: 11px;">
              <div class="mb-1" style="width: 44px; height: 44px; border-radius: 50%; background: #262626; display: flex; align-items: center; justify-content: center;">
                <i class="zigry z-add-square" style="font-size: 20px;"></i>
              </div>
              <span>Add to story</span>
            </button>
            <button class="btn btn-link text-white text-decoration-none d-flex flex-column align-items-center share-to-feed-btn" style="font-size: 11px;">
              <div class="mb-1" style="width: 44px; height: 44px; border-radius: 50%; background: #262626; display: flex; align-items: center; justify-content: center;">
                <i class="zigry z-add-rounded" style="font-size: 20px;"></i>
              </div>
            ...
            <button class="btn btn-link text-white text-decoration-none d-flex flex-column align-items-center send-message-btn" style="font-size: 11px;" disabled>
              <div class="mb-1" style="width: 44px; height: 44px; border-radius: 50%; background: #25D366; display: flex; align-items: center; justify-content: center;">
                <i class="zigry z-chat" style="font-size: 20px;"></i>
              </div>
              <span>Send</span>
            </button>
...
if(mediaData.videos){mediaData.videos.forEach((vid)=>{const posterAttr=vid.thumb?`poster="${vid.thumb}"`:"";const streamUrl=`/api/video/stream?path=${encodeURIComponent(
            vid.url,
          )}`;mediaHtml+=`<div class="zigry-video-wrapper border border-solid" style="position:relative;background-color: rgb(0, 0, 0);"><video class="w-100 zigry-video z-2" playsinline ${posterAttr} src="${streamUrl}" preload="metadata">Your browser does not support video.</video></div>`})}}catch(e){mediaHtml=post.media||""}
div.innerHTML=`
                ...
                <a href="/${
                  ...
                  <img onerror="if(!this.dataset.fallbackAttempted){this.dataset.fallbackAttempted='1';this.src='/assets/images/default/${
                    post.gender?.toLowerCase() || "756e6b6e6f776e"
                       ...
                       class="rounded-circle border border-2 object-fit-cover lock dp${post.ref_id}"
...
alt="Profile" height="32px" src="${
          ...
          <div class="count d-flex justify-content-between">
            <div class="like_count w-100 ms-4 text-start">${
              post.like_counts || "0"
            }</div>
            <div class="comments_count w-100 mx-auto text-center">${
              post.comment_counts || "0"
            }</div>
            <div class="share_count w-100 me-4 text-end">${
              post.share_counts || "0"
            }</div>
          </div>
...
const mediaContainer=div.querySelector(".media-files.media.zigry-images");if(mediaContainer){const imgs=mediaContainer.querySelectorAll("img");const videos=mediaContainer.querySelectorAll(".zigry-video-wrapper");const imgCount=imgs.length;const videoCount=videos.length;if(imgCount>0){mediaContainer.classList.add("zigry-gallery");if(imgCount===1){mediaContainer.classList.add("zigry-single");mediaContainer.classList.add("w-100");mediaContainer.style.height="40vh";mediaContainer.style.overflow="hidden";mediaContainer.style.position="relative"}else if(imgCount===2){mediaContainer.classList.add("zigry-two")}else{mediaContainer.classList.add("zigry-four");mediaContainer.classList.add("gap-1")}
imgs.forEach((img,i)=>{const wrapper=document.createElement("div");wrapper.className="zigry-img-wrap d-flex justify-content-center border border-solid";if(imgCount===1){wrapper.style.height="auto";wrapper.style.width="100%";wrapper.style.position="relative";wrapper.style.overflow="hidden";wrapper.classList.remove("d-flex","justify-content-center");wrapper.style.backgroundColor="#000";img.classList.add("zigry-single-img");img.style.objectFit="contain";img.style.height="100%";img.style.position="relative";img.style.zIndex="2";const blur=document.createElement("div");blur.className="zigry-blur-bg";blur.style.cssText=`
...
function zScroll(){zigry.applyScroll=(el)=>{if(el.dataset.zigryScrollApplied){if(el._zscroll)el._zscroll.u();return}
if(el.scrollHeight<=el.clientHeight)return;el.style.scrollbarWidth="none";el.style.msOverflowStyle="none";el.style.overflowY="auto";const unique="zs_"+Math.random().toString(36).slice(2,8);el.classList.add(unique);const st=document.createElement("style");st.textContent=`
...
let imageCard="";const isInternal=url.includes(window.location.hostname);if(data.image){if(isInternal){imageCard=`
    ...
    `}else{imageCard=`
      <div class="og-image-container d-flex justify-content-center">
        <img 
          src="${data.image}"
          class="img-thumbnail og-external-image"
          alt=""
...
const dpupdateEl=document.getElementById("dpupdate");if(dpupdateEl){const uid=dpupdateEl.getAttribute("uid");if(uid){document.querySelectorAll("."+uid).forEach((el)=>dpupdate(el,cropped))}}}
function dpupdate(el,src){if(el.tagName==="IMG"){el.src=src}else{el.style.backgroundImage=`url(${src})`}}
...
document.addEventListener("DOMContentLoaded",init);document.addEventListener("click",async(e)=>{const btn=e.target.closest(".zigry-toggle-btn");if(!btn)return;const action=btn.getAttribute("data-action");if(!action)return;e.preventDefault();zigry.loader(!0);const csrfMeta=document.querySelector('meta[name="csrf-token"]');const csrf=csrfMeta?.content??"";try{const r=await fetch(action,{method:"POST",headers:{"X-Requested-With":"Zigry-Ajax","X-CSRF-Token":csrf,},});const p=await r.json();if(p?.alert)zalert(p.alert,p.type??"error",p.position??"top-right");if(p?.props)zigry.mount(null,p.props);zigry.loader(!1)}catch(err){console.error("Toggle failed",err);zigry.loader(!1);zalert("Action failed","error","center")}});document.addEventListener("DOMContentLoaded",()=>{const COOKIE_CONSENT_KEY="zigry_cookie_consent";const banner=document.getElementById("cookie-consent-banner");if(!banner)return;const acceptBtn=document.getElementById("cookie-consent-accept");const declineBtn=document.getElementById("cookie-consent-decline");function updateConsent(granted){window.dataLayer=window.dataLayer||[];dataLayer.push({event:"consent_update",analytics_storage:granted?"granted":"denied",ad_storage:granted?"granted":"denied",ad_user_data:granted?"granted":"denied",ad_personalization:granted?"granted":"denied",})}
function setConsentCookie(value){const expiry=new Date();expiry.setFullYear(expiry.getFullYear()+1);document.cookie=`${COOKIE_CONSENT_KEY}=${value}; expires=${expiry.toUTCString()}; path=/; SameSite=Lax`;banner.classList.add("d-none")}
const existing=document.cookie.split("; ").find((c)=>c.startsWith(COOKIE_CONSENT_KEY+"="));if(existing){const val=existing.split("=")[1];updateConsent(val==="granted")}else{banner.classList.remove("d-none")}
