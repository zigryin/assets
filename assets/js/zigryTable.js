function zigryTable(tableId, baseUrl, options = {}) {
  ...
  // Cleanup any lingering filter buttons from previous SPA navigations
  const existingButtons = document.getElementById(`${tableId}_filter_buttons`);
  ...
  const createFilterButton = (def) => {
    ...
    btn.type = "button";
    btn.className =
      "btn btn-sm btn-outline-light d-flex align-items-center gap-2";
  ...
  const renderFilterButtons = (buttons, targetElement) => {
    try {
      ...
      if (document.getElementById(`${tableId}_filter_buttons`)) return;
  ...
  const updateFilterButtons = () => {
    try {
      ...
      nodes.forEach((n) => {
        ...
        n.classList.toggle("btn-zigry", active);
  ...
  const updateBulkActions = () => {
    ...
    exportCSV.textContent = currentTranslations.export_csv || "Export CSV";
    ...
    exportXLSX.textContent = currentTranslations.export_xlsx || "Export XLSX";
    ...
    exportPDF.textContent = currentTranslations.export_pdf || "Export PDF";
    exportPDF.className = "btn btn-sm btn-outline-danger me-2";
    exportPDF.onclick = () => window[`${tableId}_exportPDF`]();

    const delBtn = document.createElement("button");
    delBtn.textContent =
      currentTranslations.delete_selected || "Delete Selected";
    delBtn.className = "btn btn-sm btn-danger";
    delBtn.onclick = () => {
      let confirmMsg = currentTranslations.delete_confirm
        ? currentTranslations.delete_confirm.replace(":count", selectedIds.size)
  ...
  function fetchData(page = currentPage) {
    return fetch(buildUrl(page), {
      ...
      .then((json) => {
        ...
        currentTranslations =
          json?.attributes?.translations || currentTranslations;
        return json;
      });
  }
  ...
  function buildStaticUI(json) {
    ...
    searchInput.placeholder = currentTranslations?.search || "Search...";
    ...
    body.className = classMap.body || "card-body p-0 zigry-thumb";
    ...
    cbMaster.addEventListener("change", (e) => {
      const isChecked = e.target.checked;
      const bdy = e.target.closest("table")?.querySelector("tbody") || tbody;
      const rows = Array.from(bdy.querySelectorAll("tr"));
      ...
      rows.forEach((tr) => {
        ...
        const itemId = cb.getAttribute("data-id") || cb.dataset.id;

        if (isChecked) {
          tr.classList.add("table-active");
          if (itemId) selectedIds.add(String(itemId));
        } else {
          tr.classList.remove("table-active");
          if (itemId) selectedIds.delete(String(itemId));
        }
      });
    ...
    if (actions) {
      ...
      thA.textContent =
        theadTitles["actions"] || currentTranslations?.actions || "Actions";
    ...
    const mk = (v, label) => {
      ...
      b.className = `btn btn-sm ${
        currentView === v ? "btn-zigry" : "btn-outline-secondary"
      }`;
      b.textContent = label;
  ...
  function fillRows(items) {
    ...
    if (isMobile) {
          ...
          .forEach((h) => {
            ...
            if (formatters[h]) {
              ...
              if (typeof fmt === "string") {
                ...
                value.appendChild(fmt);
              } else {
                value.textContent = String(fmt ?? "");
              }
            } else {
      ...
      if (!items.length) {
        ...
        td.textContent = currentTranslations.no_records || "No records found.";
        tr.appendChild(td);
        tbody.appendChild(tr);
        return;
      }
  ...
  function fillGrid(items) {
    try {
      ...
      if (!items.length) {
        gridContainer.innerHTML = `<div class="text-center text-muted p-3">${currentTranslations.no_records || "No records found."}</div>`;
        return;
      }
      ...
      if (style === "compact") {
        ...
        items.forEach((item) => {
          ...
          details.innerHTML = detailFields
            .map(
              (h) =>
                `<strong>${theadTitles[h] || h}:</strong> ${item[h] ?? ""}`,
            )
            .join("<br>");
          body.appendChild(details);
  ...
  function fillList(items) {
    try {
      ...
      if (!items.length) {
        listContainer.innerHTML = `<div class="list-group-item text-center text-muted">${currentTranslations.no_records || "No records found."}</div>`;
        return;
      }
  ...
  function updatePagination(json = {}) {
    ...
    let showingString = currentTranslations?.showing_x_of_y
      ? currentTranslations.showing_x_of_y
          .replace(":start", start)
          .replace(":end", end)
          .replace(":total", total)
      : `Showing ${start}–${end} of ${total}`;
    info.textContent = showingString;
  ...
  setTimeout(() => {
    try {
      let target = tableElement;
      if (!target.isConnected) {
        const current = document.getElementById(tableId);
        if (current) target = current;
      }

      const fallbackTarget =
        target.closest(".datatable-container") || target.parentElement;

      if (fallbackTarget && tb.showFilterButtons)
