...
const OUTGOING_QUEUE_KEY = `zigry_outgoing_queue_${window.me?.id || "anon"}`;

function loadOutgoingQueue() {
  try {
    const json = localStorage.getItem(OUTGOING_QUEUE_KEY);
    return json ? JSON.parse(json) : [];
  } catch (e) {
    console.warn("[chat] Failed to load outgoing queue", e);
    return [];
  }
}

function saveOutgoingQueue() {
  try {
    localStorage.setItem(
      OUTGOING_QUEUE_KEY,
      JSON.stringify(window._outgoingQueue),
    );
  } catch (e) {
    console.warn("[chat] Failed to save outgoing queue", e);
  }
}

window._outgoingQueue = loadOutgoingQueue();
...
function queueSend(...args) {
  ...
  saveOutgoingQueue(); // Persist to localStorage
  flushOutgoingQueue();
}
...
function flushOutgoingQueue() {
  ...
  while (window._outgoingQueue.length) {
    ...
    try {
      ...
      saveOutgoingQueue(); // Update localStorage after successful send
    } catch (e) {
...
zigrySocket.on("private", async (data) => {
  let contactId = data.from == window.me.id ? data.to : data.from;
  let contact = window.contacts.find(
    (c) => String(c.id) === String(contactId) || String(c.mobile) === String(contactId),
  );

  const now = new Date();

  // If contact is not in current contacts array, create a dynamic fallback contact so message is not lost
  if (!contact) {
    contact = {
      id: contactId,
      name: data.name || ("User #" + contactId),
      avatar: "/images/default/male",
      lastMsg: "",
      lastMsgFromId: data.from,
      time: now.toISOString(),
      timestamp: data.timestamp || now.getTime(),
      online: true,
      unread: 0,
      msgkeydec: data.msgkeydec || (window.me ? `zigryhash_${contactId}_${window.me.id}` : ""),
      msgkeyenc: data.msgkeyenc || (window.me ? `zigryhash_${window.me.id}_${contactId}` : "")
    };
    window.contacts.unshift(contact);
    if (typeof renderContacts === "function") renderContacts(window.contacts);
  }

  const msgs = loadMessages(contactId);

  // ✅ Deduplication: Prevent duplicate messages from multiple tabs/sync
  if (msgs.some((m) => String(m.id) === String(data.id))) return;

  // ✅ Decrypt incoming message using msgkeydec (messages FROM this contact TO me)
  let plainText = data.message;
  if (data.message && contact.msgkeydec) {
    try {
      plainText = await ZigrySocket.decrypt(data.message, contact.msgkeydec);
    } catch (e) {
      plainText = data.message;
      console.warn("[chat] Failed to decrypt private message from", contactId, e);
    }
  }
  ...
  if (window.selectedContact && String(window.selectedContact.id) === String(contactId)) {
    renderMessages(contactId);
    localStorage.setItem("unread_" + contactId, "0");
    contact.unread = 0;
    markMessagesAsSeen(contactId);
    updateContactPreview(contactId, plainText, msgObj.timestamp, 0, msgObj.from);
  } else {
    let unread = Number(localStorage.getItem("unread_" + contactId)) || (contact.unread || 0);
  ...
  if (contact) {
    contact.lastMsg = plainText;
    contact.lastMsgFromId = msgObj.from;
    contact.timestamp = msgObj.timestamp;
  }

  // Live header update if header helper function exists
  if (typeof updateHeaderLive === "function") {
    try {
      updateHeaderLive({
        from: data.from,
        to: data.to,
...
zigrySocket.on("group", async (data) => {
  const groupId = data.room;
  const group = window.groups.find((g) => g.id == groupId);
  if (!group) return;

  // ✅ Decrypt incoming group message using the shared group key.
  let plainText = data.message;
  if (data.message && group.groupkey) {
    try {
      plainText = await ZigrySocket.decrypt(data.message, group.groupkey);
    } catch (e) {
      plainText = "[Encrypted message]";
      console.warn("[chat] Failed to decrypt group message in group", groupId, e);
    }
  }

  const msgs = loadGroupMessages(groupId);

  // Deduplication & ignore own messages if echoed
  if (data.from == window.me.id || msgs.some((m) => m.id == data.id)) return;
...
function loadMessages(contactId) {
  if (window.messageCache.contacts[contactId]) {
    msgs = window.messageCache.contacts[contactId];
    if (msgs !== "[]") {
      return msgs;
    }
  }
...
async function selectContactOrGroup(obj, isGroupContact, updateQuery = false) {
  ...
  // 2. Fetch latest messages from server (background sync to keep UI responsive with cached data)
  const contactId = obj.id;
  const cachedMessages = isGroupContact
    ? loadGroupMessages(contactId)
    : loadMessages(contactId);

  // If no cache, await fetch; if cached, fetch in background and re-render
...
async function renderMessages(
  ...
  msgs.forEach((msg) => {
    ...
    // Check if this is a share message (type = 'share')
    if (msg.type === "share" && msg.text) {
      try {
        const shareData = JSON.parse(msg.text);

        // Fix reel thumbnail path if it's just a filename
        let thumbUrl = shareData.thumbnail;
        if (
          shareData.content_type === "reel" &&
          thumbUrl &&
          !thumbUrl.includes("/") &&
          !thumbUrl.startsWith("http")
        ) {
          thumbUrl = `/uploads/reels/${msg.from_id}/${thumbUrl}`;
        }

        // Render rich card for share
        bubble.innerHTML = `
          <div class="share-card" style="max-width: 320px; border-radius: 8px; overflow: hidden; background: #2a2a2a;">
            ...
            <div style="padding: 12px;">
              ...
              <div style="color: #fff; font-size: 14px; margin-bottom: 10px; font-weight: 500;">
                ${zigry.escapeHtml(shareData.message)}
              </div>
              `
                  : ""
              }
              <div style="color: #b0b0b0; font-size: 13px; margin-bottom: 8px;">
                ${shareData.preview_text || (shareData.content_type === "reel" ? "Shared a reel" : "Shared a post")}
              </div>
              <a href="${shareData.url}" 
                 class="btn btn-sm btn-primary w-100" 
                 target="_blank"
                 style="text-decoration: none;">
                View ${shareData.content_type === "reel" ? "Reel" : "Post"}
              </a>
            </div>
          </div>
        `;
        updateContactPreview(
          contactId,
          `📤 Shared a ${shareData.content_type === "reel" ? "reel" : "post"}`,
          msg.timestamp,
          0,
        );
      } catch (e) {
        console.error("Failed to parse share message:", e);
        bubble.innerText = msg.text; // Fallback to raw text
      }
    }
...
function getMessageStatusIcon(msg) {
  ...
  const status = String(msg.status || "4"); // Default to '4' (pending) if undefined
  if (status === "0")
  ...
  if (status === "4") {
    return `<span class="msg-status text-muted" title="Sending..." data-status="4">⏱</span>`; // Clock for Pending
  }
...
function markMessagesAsSeen(contactId) {
  localStorage.setItem("unread_" + contactId, "0");
  const contact = window.contacts.find((c) => String(c.id) === String(contactId));
  if (contact) contact.unread = 0;

  const li = document.querySelector(`.contact-item[data-id="${contactId}"]`);
  if (li) {
    const badge = li.querySelector(".unread-badge") || li.querySelector(".unread");
    if (badge) badge.remove();
  }
...
function initChatFeatures() {
  ...
  // BUT reattach if the DOM has been cleared/replaced (SPA navigation)
  const form = document.getElementById("chat-form");
  if (!form) return;

  // Check if listener is already attached by checking for our custom flag
  if (form._chatListenerAttached) {
    return; // Skip if already initialized on this specific form element
  }
  form._chatListenerAttached = true;

  // --- Tab switching ---
  const tabDirect = document.getElementById("tab-direct");
  const tabGroup = document.getElementById("tab-group");

  if (tabDirect && !tabDirect._clickListenerAttached) {
    ...
    tabDirect._clickListenerAttached = true;
  }

  if (tabGroup && !tabGroup._clickListenerAttached) {
    ...
    tabGroup._clickListenerAttached = true;
  }
    ...
    if (text) {
      try {
        ...
        const msgObj = {
          ...
          text: encryptedText,
          timestamp: now,
          status: "4", // Pending - will update to "2" when server confirms sent
        };
...
function initChatApp() {
  ...
  // Load messages if a contact is pre-selected (SPA navigation fix)
  if (window.selectedContact && window.selectedContact.id) {
    renderMessages(window.selectedContact.id);
  }
}
...
/**
 * Retry sending pending messages (status "4") from localStorage on socket reconnect
 * This ensures messages aren't lost if user closes tab before they're sent
 */
function retryPendingMessages() {
  ...
  // Scan all localStorage keys for message caches
  Object.keys(localStorage).forEach((key) => {
    if (!key.startsWith(`zigry_msg_${myId}_c_`)) return; // Only contact messages

    try {
      const msgs = JSON.parse(localStorage.getItem(key));
      if (!Array.isArray(msgs)) return;

      const contactId = key.replace(`zigry_msg_${myId}_c_`, "");
      const pendingMsgs = msgs.filter(
        (m) => m.status === "4" && m.from == myId,
      );

      pendingMsgs.forEach((msg) => {
        // Skip if already in queue
        if (window._outgoingQueue.some((q) => q.id === msg.id)) return;
        if (window._outgoingPendingIds.has(msg.id)) return;
      ...
      console.warn("[chat] Failed to scan pending messages", e);
    }
  });

  if (retried > 0) {
    console.log(`[chat] Retrying ${retried} pending message(s)`);
  }
}
...
zigrySocket.on("open", () => {
  ...
  // Retry pending messages from localStorage
...
function updateContactPreview(
  ...
  if (contact) {
    ...
    if (fromId) contact.lastMsgFromId = fromId;
    if (time) contact.timestamp = time;
    if (unread !== null) contact.unread = unread;
  }

  const li = document.querySelector(`.contact-item[data-id="${contactId}"]`);
  if (li) {
    const lastMsgDiv = li.querySelector(".contact-last-msg");
    const lastTimeDiv = li.querySelector(".date");
    ...
    let badge = li.querySelector(".unread-badge") || li.querySelector(".unread");
    if (unread === null)
      unread = Number(localStorage.getItem("unread_" + contactId)) || (contact ? contact.unread : 0);

    if (unread > 0) {
      if (!badge) {
        badge = document.createElement("span");
        badge.className = "unread-badge unread badge rounded-pill bg-primary mt-1";
  ...
  if (typeof updateHeaderLive === "function" && fromId) {
    try {
      updateHeaderLive({
        from: fromId,
        to: window.me ? window.me.id : null,
        message: lastMsg
      });
    } catch (e) {}
  }
...
async function uploadFileAndSend(file, isGroup = false) {
  ...
  if (!contactId) {
    zigry.alert({
      ...
      message: "Please select a contact or group before sending files.",
      type: "warning",
      duration: 3000,
    });
    return;
...
// File input handler — delegated so it works after SPA/AJAX navigation
document.addEventListener("click", function (e) {
  if (e.target && e.target.closest("#chat-file-btn")) {
